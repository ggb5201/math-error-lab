...
【统一输出规则】
1. 输出结构化纯文本，适合直接复制打印；
2. 解题步骤为考场可直接执行的流程，每一步写明操作及目的；
3. 严格遵循高考范围，拒绝超纲内容；
4. 错因仅限五选一：概念不清 / 计算失误 / 审题遗漏 / 套路不会 / 思维漏洞；
5. 语言简洁专业，不说废话；
6. 数学符号全部用纯文本表示，禁止使用 LaTeX 语法（不要出现 $、\\in、\\mathbf、\\frac、\\sqrt、\\overrightarrow 等命令）。例如：x² 写 x^2，分数写 a/b，根号写 sqrt(x)，向量写 CA·CB，属于正整数集写 n∈N*。"""
...
OCR_PROMPT = """请识别图片中的数学题目文字，完整准确地转录为纯文本。

要求：
1. 只输出题目原文，不要添加任何解释、标注或格式化符号；
2. 数学公式用纯文本表示（如 x² 写作 x^2，分数 a/b，根号写作 sqrt(x)）；
3. 保持原题的换行和段落结构；
4. 如果图片中有多道题，每道题之间用空行分隔；
5. 忽略图片中的页码、水印等非题目内容；
6. 如果图片模糊或无法识别，直接输出"识别失败"三个字；
7. 不要使用 LaTeX 语法（不要出现 $、\\in、\\mathbf、\\frac 等符号），全部用普通文本表示。"""
